...
export const useConversationState = (
  ...
  const handleMessageFeedback = async (messageIndex: number, rating: number) => {
    if (!currentConversationId || !restaurantId || !userId) return;
    ...
    try {
      ...
      if (fetchError || !msgs || !msgs[messageIndex]) {
        console.error("Error fetching messages for feedback:", fetchError);
        return;
      }
  ...
  const handleLoadConversation = async (conversationId: string) => {
    try {
      ...
      setConversationMessages(msgs.map(msg => ({
        id: msg.id,
